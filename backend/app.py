...
# Defaults to '*' (allow all origins). In production, ALLOWED_ORIGINS can be set
# to comma-separated URLs, e.g. "https://my-app.vercel.app,https://my-app.netlify.app"
...
# Configuration: Deployment-safe relative paths for models and uploads
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

default_image_model = os.path.join(BASE_DIR, 'model', 'FreshGuard_Good_Bad_MobileNetV2.keras')
IMAGE_MODEL_PATH = os.environ.get('IMAGE_MODEL_PATH', default_image_model)

# Resolve Random Forest model with fallback for Linux filesystem case sensitivity
rf_candidate = os.path.join(BASE_DIR, 'model', 'freshguard_random_forest.pkl')
if not os.path.exists(rf_candidate):
    rf_candidate = os.path.join(BASE_DIR, 'model', 'FreshGuard_random_forest.pkl')
RF_MODEL_PATH = os.environ.get('RF_MODEL_PATH', rf_candidate)
...
def predict_manual():
    ...
    for field, valid_categories in SUPPORTED_CATEGORIES.items():
        ...
        # Case-insensitive category lookup to match exact model category casing
...
if __name__ == '__main__':
    # Run the Flask server with configurable port and debug settings for deployment
